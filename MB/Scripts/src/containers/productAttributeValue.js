...
var ProductAttributeValue = React.createClass({
  ...
  onEdit(record){
    this.props.productAttributeValueActions.getById(record.id).then((err)=> {
      if (err) {
        message.error('获取ProductAttributeValue数据失败！请刷新页面尝试。');
      }
      else {
        this.setState({
          visible: true,
          edit: true,
          title: '编辑ProductAttributeValue'
        });
      }
    });
  },
  ...
  onRemove(record){
    ...
    confirm({
      title: '确认删除该ProductAttributeValue？',
      onOk() {
  ...
  render() {
    ...
    const { productAttributeValue:{ loading, list, entity }} = this.props;
      ...
      <div className='container'>
        <div className='ant-list-header' data-flex="dir:right">
          <div className='ant-list-header-right'>
            <Button type="primary" icon="plus" onClick={this.onAdd}>添加ProductAttributeValue</Button>
          </div>
        </div>
        <Table
          ref='table'
...
const statics = {
  ...
  breadcrumb: [{
    ...
    title: 'ProductAttributeValue管理'
  }]
};

ProductAttributeValue = createForm()(ProductAttributeValue);

export default connectStatic(statics)(connect(mapStateToProps, mapDispatchToProps)(ProductAttributeValue))
