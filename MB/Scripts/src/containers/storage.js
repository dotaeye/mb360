...
var Storage = React.createClass({
  ...
  fetchData(page){
    this.props.storageActions.getAll({
  ...
  onEdit(record){
    this.props.storageActions.getById(record.id).then((err)=> {
      if (err) {
        message.error('获取Storage数据失败！请刷新页面尝试。');
      }
      else {
        this.setState({
          visible: true,
  ...
  onModalSubmit(){
    ...
    const { update, create} =this.props.storageActions;
    const { entity }= this.props.storage;
...
export default connectStatic(statics)(connect(mapStateToProps, mapDispatchToProps)(Storage))
