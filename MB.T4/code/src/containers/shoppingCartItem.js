...
import * as shoppingCartItemActions from '../actions/shoppingCartItem'
...
var ShoppingCartItem = React.createClass({
  ...
  fetchData(page){
    this.props.shoppingCartItemActions.getAll({
  ...
  onAdd(){
    this.setState({
      visible: true,
      edit: false,
      title: '添加ShoppingCartItem',
      record: {}
    });
  },

  onEdit(record){
    this.props.shoppingCartItemActions.getById(record.id).then((err)=> {
      if (err) {
        message.error('获取ShoppingCartItem数据失败！请刷新页面尝试。');
      }
      else {
        this.setState({
          visible: true,
          edit: true,
          title: '编辑ShoppingCartItem'
        });
      }
    });
  },
  ...
  onRemove(record){
    const self = this;
    const { shoppingCartItem:{ list }} = this.props;
    let source = list.data;
    const remove = this.props.shoppingCartItemActions.remove;
    confirm({
      title: '确认删除该ShoppingCartItem？',
      onOk() {
  ...
  onModalSubmit(){
    ...
    const { update, create} =this.props.shoppingCartItemActions;
    const { entity }= this.props.shoppingCartItem;
  ...
  render() {
    ...
    const { shoppingCartItem:{ loading, list, entity }} = this.props;
      ...
      <div className='container'>
        <div className='ant-list-header' data-flex="dir:right">
          <div className='ant-list-header-right'>
            <Button type="primary" icon="plus" onClick={this.onAdd}>添加ShoppingCartItem</Button>
          </div>
        </div>
        <Table
          ref='table'
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    shoppingCartItemActions: bindActionCreators(shoppingCartItemActions, dispatch)
  }
}
...
const statics = {
  ...
  breadcrumb: [{
    ...
    title: 'ShoppingCartItem管理'
  }]
};

ShoppingCartItem = createForm()(ShoppingCartItem);

export default connectStatic(statics)(connect(mapStateToProps, mapDispatchToProps)(ShoppingCartItem))
