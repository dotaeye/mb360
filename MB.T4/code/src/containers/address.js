...
var Address = React.createClass({
  ...
  fetchData(page){
    this.props.addressActions.getAll({
  ...
  onEdit(record){
    this.props.addressActions.getById(record.id).then((err)=> {
      if (err) {
        message.error('获取Address数据失败！请刷新页面尝试。');
      }
      else {
        this.setState({
          visible: true,
  ...
  onRemove(record){
    const self = this;
    const { address:{ list }} = this.props;
    let source = list.data;
    const remove = this.props.addressActions.remove;
    confirm({
      title: '确认删除该Address？',
      onOk() {
  ...
  onModalSubmit(){
    ...
    const { update, create} =this.props.addressActions;
    const { entity }= this.props.address;
  ...
  render() {
    ...
    const { address:{ loading, list, entity }} = this.props;
      ...
      <div className='container'>
        <div className='ant-list-header' data-flex="dir:right">
          <div className='ant-list-header-right'>
            <Button type="primary" icon="plus" onClick={this.onAdd}>添加Address</Button>
          </div>
        </div>
        <Table
          ref='table'
...
export default connectStatic(statics)(connect(mapStateToProps, mapDispatchToProps)(Address))
