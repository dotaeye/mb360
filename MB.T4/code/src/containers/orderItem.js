...
var OrderItem = React.createClass({
  ...
  fetchData(page){
    this.props.orderItemActions.getAll({
  ...
  onEdit(record){
    this.props.orderItemActions.getById(record.id).then((err)=> {
      if (err) {
        message.error('获取OrderItem数据失败！请刷新页面尝试。');
      }
      else {
        this.setState({
          visible: true,
  ...
  onRemove(record){
    const self = this;
    const { orderItem:{ list }} = this.props;
    let source = list.data;
    const remove = this.props.orderItemActions.remove;
    confirm({
      title: '确认删除该OrderItem？',
      onOk() {
  ...
  onModalSubmit(){
    ...
    const { update, create} =this.props.orderItemActions;
    const { entity }= this.props.orderItem;
  ...
  render() {
    ...
    const { orderItem:{ loading, list, entity }} = this.props;
      ...
      <div className='container'>
        <div className='ant-list-header' data-flex="dir:right">
          <div className='ant-list-header-right'>
            <Button type="primary" icon="plus" onClick={this.onAdd}>添加OrderItem</Button>
          </div>
        </div>
        <Table
          ref='table'
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    orderItemActions: bindActionCreators(orderItemActions, dispatch)
  }
}
...
OrderItem = createForm()(OrderItem);

export default connectStatic(statics)(connect(mapStateToProps, mapDispatchToProps)(OrderItem))
