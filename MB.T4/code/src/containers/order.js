...
var Order = React.createClass({
  ...
  fetchData(page){
    this.props.orderActions.getAll({
  ...
  onEdit(record){
    this.props.orderActions.getById(record.id).then((err)=> {
      if (err) {
        message.error('获取Order数据失败！请刷新页面尝试。');
      }
      else {
        this.setState({
          visible: true,
  ...
  onRemove(record){
    ...
    const remove = this.props.orderActions.remove;
    confirm({
      title: '确认删除该Order？',
      onOk() {
  ...
  onModalSubmit(){
    ...
    const { update, create} =this.props.orderActions;
    const { entity }= this.props.order;
  ...
  render() {
    ...
    const { order:{ loading, list, entity }} = this.props;
      ...
      <div className='container'>
        <div className='ant-list-header' data-flex="dir:right">
          <div className='ant-list-header-right'>
            <Button type="primary" icon="plus" onClick={this.onAdd}>添加Order</Button>
          </div>
        </div>
        <Table
          ref='table'
